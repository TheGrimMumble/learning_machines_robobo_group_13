...
class RoboboGymEnv(gym.Env):
    ...
    def get_reward(self, obs, left_speed, right_speed):
        IR_sensors = obs[:8]
        IR_front = np.array([IR_sensors[2], IR_sensors[3], IR_sensors[4], IR_sensors[5], IR_sensors[7]])
        green_found, cx, cy, area, food = obs[8:]

        reward = 0.0
        collision = np.max(IR_front) > self.collision_threshold  # e.g., 300

        # --- Collision penalty (only if food not seen) ---
        if collision and green_found == 0:
            reward -= 3.0

        # --- Forward speed reward (only when food is seen) ---
        if green_found == 1:
            speed_norm = max(0.0, (left_speed + right_speed) / 200.0)
            turn_penalty = abs(left_speed - right_speed) / 200.0
            forwardness = 1.0 - turn_penalty
            reward += 0.1 * (speed_norm * forwardness)

            # --- Alignment reward: encourages robot to center on food (cx = 0.5 ideal) ---
            alignment_score = 1.0 - abs(cx - 0.5) * 2  # [1.0 if centered, 0.0 if edge]
            reward += 0.2 * alignment_score

        # --- Time-weighted food collection reward ---
        new_food_collected = self.nr_collected_food - self.prev_nr_collected_food
        if new_food_collected > 0:
            time_factor = 1.0 - (self.step_in_episode / self.max_steps_in_episode)
            reward += new_food_collected * (1.0 * time_factor)

        # --- Early completion bonus (if all food collected quickly) ---
        if self.nr_collected_food >= self.total_count_food:
            reward += 5.0
            self.done = True
    ...
    #     # Only apply collision penalty when green is NOT found
    #     if green_found == 0:
    #         max_ir = np.max(IR_sensors_front)
    #         if max_ir > self.collision_threshold:
    #             proximity_penalty = max_ir * self.weight_dict['IR_cost_weight']
    #             reward -= proximity_penalty
    #     else:
    #         # Alignment bonus (cx near 0.5 is centered)
    #         alignment_reward = 1.0 - abs(cx - 0.5) * 2
    #         reward += 0.1
    #         reward += alignment_reward * self.weight_dict['alignment_weight']

    #         # Forward speed only matters when green is found
    #         forward_speed_reward = self.forward_speed_reward()
    #         reward += forward_speed_reward * self.weight_dict['forward_speed_weight']
    #     # (optional) also reward speed even when not seeing green? Then move this outside the if-else

    #     # Area reward (encourages closeness)
    #     reward += area * self.weight_dict['area_weight']

    #     # Reward for food collection progress
    #     new_food = self.nr_collected_food - self.prev_nr_collected_food
    #     if new_food > 0:
    #         # Optional time-based shaping
    #         time_factor = 1.0 - (self.step_in_episode / self.max_steps_in_episode)
    #         reward += new_food * self.weight_dict['food_weight'] * time_factor

    #     # Debug / logging
    #     self.green_found = green_found
    #     self.proximity_penalty = proximity_penalty
    #     self.forward_speed_reward_ = forward_speed_reward if green_found else 0.0
