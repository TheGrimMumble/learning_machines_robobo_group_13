...
class Individual:
    ...
    def copy(self):
        ...
        return new
#TO DO: STEP 1: Approach red object and get it in gripper
#STEP 2: line up the red object and green area
def fitness_evaluation(
    ...
    rob.set_phone_tilt_blocking(95, 100)
    detector.current_pan = 177
    detector.current_tilt = 95
    ...
    while time.time() - start_time < max_time:
        ...
        if found_red and found_green:
            ...
            # Output when both objects are detected
            print(f"Both objects detected! Red x: {rx:.3f}, Green x: {gx:.3f}, Alignment: {alignment:.3f}")
        
        elif found_red:
            print("Only red object detected")
        elif found_green:
            print("Only green object detected")
        else:
    ...
    individual.fitness = alignment_sum * 10.0 - collision_penalty
...
def genetic_algorithm(rob: IRobobo):
    ...
    for i, individual in enumerate(population):
        ...
        print(f"Individual {i+1}/{MU}: fitness={fitness:.2f}")
    ...
    for generation in range(GENERATIONS):
        ...
        for i, individual in enumerate(offspring):
            ...
            if (i + 1) % 10 == 0 or i == len(offspring) - 1:
                print(f"Offspring {i+1}/{len(offspring)}: fitness={fitness:.2f}")
        ...
        if current_best.fitness > best_fitness:
            best_individual = current_best.copy()
            best_fitness = current_best.fitness
            print(f"NEW BEST in generation {generation + 1}: fitness={best_fitness:.2f}")

        avg_fitness = sum(ind.fitness for ind in population) / len(population)
        print(f"Generation {generation + 1} stats:")
        print(f"  Best: {population[0].fitness:.2f} ")
        print(f"  Average: {avg_fitness:.2f}")
        print(f"  Worst: {population[-1].fitness:.2f}")
        print(f"  Overall best: {best_fitness:.2f}")
